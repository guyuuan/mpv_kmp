...
nativeprefix () {
    local arch="$1"
    # prefer new android-<arch> prefix, fallback to legacy
    if [ -f "$BUILD/prefix/android-$arch/lib/libmpv.so" ]; then
        echo "$BUILD/prefix/android-$arch"
    elif [ -f "$BUILD/prefix/$arch/lib/libmpv.so" ]; then
        echo "$BUILD/prefix/$arch"
	else
