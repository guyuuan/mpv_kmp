package com.guyuuan.kmp.mpv.gradle
    ...
    fun extract() {
        ...
        NativeLibraries.applePlatforms.forEach { platform ->
            ...
            NativeLibraries.names.forEach { name ->
                val resourcePath = "com/guyuuan/kmp/mpv/nativeLibs/$platform/$name"
                val target = outputDir.resolve(name)
                javaClass.classLoader.getResourceAsStream(resourcePath).use { input ->
                    if (input == null) {
...
private fun Project.nativeArchiveFiles(
    ...
    val dependency = dependencies.create(
        "com.guyuuan.kmp.mpv:mpv-gradle-plugin:$pluginVersion:$classifier@zip"
    )
...
private object DesktopNativeLibraries {
    const val resourceRoot = "com/guyuuan/kmp/mpv/desktopNativeLibs"
